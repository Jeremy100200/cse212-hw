    ...
    public void Remove(int value)
    {
        ...
        while (curr is not null)
        {
            if (curr.Data == value)
            {
                ...
                // if it's the tail, reuse the RemoveTail function to update the tail and disconnect the last node
                else if (curr == _tail)
                {
                    RemoveTail();
                }
                //otherwise, it's in the middle of the list, so we need to connect the previous node to the next node to remove the current node
                else
                {
